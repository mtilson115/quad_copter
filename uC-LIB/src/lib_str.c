                              ...
                              CPU_SIZE_T   len_max)
{
    ...
    while (( pstr_len != (const CPU_CHAR *)  0 ) &&             /* Calc str len until NULL ptr (see Note #3a) ...       */
           (*pstr_len != (      CPU_CHAR  )'\0') &&             /* ... or NULL int8_t found      (see Note #3b) ...       */
                              ...
                              CPU_SIZE_T   len_max)
{
    ...
    while (( pstr_copy_dest != (      CPU_CHAR *)  0 ) &&       /* Copy str until NULL ptr(s)  [see Note #3b]  ...      */
           ...
           (*pstr_copy_src  != (      CPU_CHAR  )'\0') &&       /* ... or NULL int8_t found      (see Note #3c); ...      */
    ...
    if (len_copy < len_max) {                                   /* If  copy str len < max buf len (see Note #2a2A), ... */
       *pstr_copy_dest = (CPU_CHAR)'\0';                        /* ... copy NULL int8_t  (see Note #3c1).                 */
    }
                             ...
                             CPU_SIZE_T   len_max)
{
    ...
    while (( pstr_cat_dest != (CPU_CHAR *)  0 ) &&              /* Adv to end of cur dest str until NULL ptr ...        */
           (*pstr_cat_dest != (CPU_CHAR  )'\0')) {              /* ... or NULL int8_t found..                             */
        pstr_cat_dest++;
    }
    ...
    while (( pstr_cat_dest != (      CPU_CHAR *)  0 ) &&        /* Cat str until NULL ptr(s)  [see Note #3b]  ...       */
           ...
           (*pstr_cat_src  != (      CPU_CHAR  )'\0') &&        /* ... or NULL int8_t found     (see Note #3c); ...       */
   ...
   *pstr_cat_dest = (CPU_CHAR)'\0';                             /* Append NULL int8_t    (see Note #2a2C).                */
                              ...
                              CPU_SIZE_T   len_max)
{
    ...
    if (*p1_str_cmp != *p2_str_cmp) {                           /* If strs NOT identical, ...                           */
                                                                /* ... calc & rtn int8_t diff  (see Note #3c1).           */
         cmp_val = (CPU_INT16S)(*p1_str_cmp) - (CPU_INT16S)(*p2_str_cmp);

    } else if (*p1_str_cmp  == (CPU_CHAR)'\0') {                /* If NULL int8_t(s) found, ...                           */
                                        ...
                                        CPU_SIZE_T   len_max)
{
    ...
    if (char_1 != char_2) {                                     /* If strs NOT identical, ...                           */
         cmp_val = (CPU_INT16S)char_1 - (CPU_INT16S)char_2;     /* ... calc & rtn int8_t diff  (see Note #3c1).           */

    } else if (char_1 == (CPU_CHAR)'\0') {                      /* If NULL int8_t(s) found, ...                           */
         cmp_val = (CPU_INT16S)0;                               /* ... strs identical; rtn 0 (see Note #3b).            */
                              ...
                              CPU_CHAR     srch_char)
{
    ...
    while (( pstr_char != (const CPU_CHAR *)  0 )      &&       /* Srch str until NULL ptr     [see Note #3b]  ...      */
           (*pstr_char != (      CPU_CHAR  )'\0')      &&       /* ... or NULL int8_t            (see Note #3c)  ...      */
           (*pstr_char != (      CPU_CHAR  )srch_char) &&       /* ... or srch int8_t found      (see Note #3d); ...      */
    ...
    if (len_srch >= len_max) {                                  /* Rtn NULL if srch int8_t NOT found ...                  */
        return ((CPU_CHAR *)0);                                 /* ... within max nbr of chars     (see Note #3e1).     */
    }

    if (*pstr_char != srch_char) {                              /* Rtn NULL if srch int8_t NOT found (see Note #3c1).     */
         return ((CPU_CHAR *)0);
    }


    return ((CPU_CHAR *)pstr_char);                             /* Else rtn ptr to found srch int8_t (see Note #3d1).     */
}
                                   ...
                                   CPU_CHAR     srch_char)
{
    ...
    pstr_char    = pstr;
     str_len_max = len_max - sizeof((CPU_CHAR)'\0');            /* Str len adj'd for NULL int8_t len.                     */
    ...
    while (( pstr_char != pstr) &&                              /* Srch str from end until begining (see Note #3c) ...  */
           (*pstr_char != srch_char)) {                         /* ... until srch int8_t found        (see Note #3d).     */
        pstr_char--;
    }
                              ...
                              CPU_CHAR     *pstr)
{
    ...
    pstr_fmt = Str_FmtNbr_Int32(nbr,                            /* Fmt uint32_t into str.                           */
                                nbr_dig,
                                nbr_base,
                                DEF_NO,
                                lead_char,
                              ...
                              CPU_CHAR     *pstr)
{
    ...
    pstr_fmt = Str_FmtNbr_Int32((CPU_INT32U)nbr_fmt,            /* Fmt signed int32_t into str.                             */
                                            nbr_dig,
                                            nbr_base,
                                            nbr_neg,
                                            lead_char,
...
/*$PAGE*/
    ...
    if (lead_char != (CPU_CHAR)'\0') {
        ...
        if (print_char != DEF_YES) {                            /* If lead int8_t non-printable  (see Note #3a1), ...     */
            fmt_invalid = DEF_YES;                              /* ... fmt invalid str         (see Note #6d).          */

        } else if (lead_char != '0') {                          /* Chk lead int8_t for non-0 dig.                         */
            lead_char_dig = ASCII_IsDig(lead_char);
            if (lead_char_dig == DEF_YES) {                     /* If  lead int8_t     non-0 dig (see Note #3a2A), ...    */
                fmt_invalid = DEF_YES;                          /* ... fmt invalid str         (see Note #6d).          */
            }
        }
    }
    ...
    if (fmt_invalid == DEF_NO) {
        ...
        if (((nbr_dig >= (nbr_dig_max + nbr_neg_sign)) ||       /* If req'd nbr digs >= (max nbr digs + neg sign)    .. */
            ...
            lead_char_0    = (lead_char == '0')                 /* Chk if lead int8_t a '0' dig (see Note #3b2).          */
                           ?  DEF_YES : DEF_NO;
            ...
            if (nbr_dig_sig < LIB_STR_CFG_FP_MAX_NBR_DIG_SIG) { /* If nbr sig digs < max, fmt str digs;           ...   */
                ...
                if ((nbr_shiftd > 0) ||                         /* If shifted nbr > 0                          ...      */
                    ...
                    if ((nbr_neg      == DEF_YES) &&            /* If  nbr neg                     ...                  */
                        ...
                        if (lead_char_fmtd == DEF_YES) {        /* ... & if lead int8_t(s) fmt'd,    ...                  */
                            pstr_fmt--;                         /* ... replace last lead int8_t w/   ...                  */
                        }
                    ...
                    if (nbr_shiftd > 0) {                       /* If shifted nbr > 0,        ...                       */
                       ...
                       *pstr_fmt++ = '0';                       /* ... fmt one '0' int8_t (see Note #3c5).                */
                    }

                } else if ((nbr_neg      == DEF_YES) &&         /* ... else if nbr neg                         ...      */
                           (lead_char_0  == DEF_YES) &&         /* ... & lead int8_t a '0' dig                   ...      */
                           (nbr_neg_fmtd == DEF_NO )) {         /* ... but neg sign NOT yet fmt'd,             ...      */
                   ...
                   *pstr_fmt++     = lead_char;                 /* ... fmt lead int8_t.                                   */
...
/*$PAGE*/
    ...
    if (nul != DEF_NO) {                                        /* If NOT DISABLED, append NULL int8_t (see Note #4).     */
       *pstr_fmt = (CPU_CHAR)'\0';
    }
                                        ...
                                        CPU_INT08U    nbr_base)
{
    ...
    nbr = Str_ParseNbr_Int32(               pstr,               /* Parse/convert str ...                                */
                                            pstr_next,
                                            nbr_base,
                                            DEF_NO,             /* ... as uint32_t (see Note #2a2).                 */
                             (CPU_BOOLEAN *)0);
                                        ...
                                        CPU_INT08U    nbr_base)
{
    ...
    nbr_abs = Str_ParseNbr_Int32(pstr,                          /* Parse/convert str ...                                */
                                 pstr_next,
                                 nbr_base,
                                 DEF_YES,                       /* ... as signed int32_t (see Note #2a2).                   */
                                &nbr_neg);
...
/*$PAGE*/
    ...
    if (lead_char != (CPU_CHAR)'\0') {
        ...
        if (print_char != DEF_YES) {                            /* If lead int8_t non-printable (see Note #3a1), ...      */
        ...
        } else if (lead_char != '0') {                          /* Chk lead int8_t for non-0 nbr base dig.                */
            ...
            if (lead_char_dig == DEF_YES) {                     /* If lead int8_t non-0 nbr base dig (see Note #3a2A), ...*/
                fmt_invalid = DEF_YES;                          /* ... fmt invalid str             (see Note #6e).      */
            }
        }
    }
    ...
    if (fmt_invalid == DEF_NO) {
        ...
        if (nbr_dig >= (nbr_dig_max + nbr_neg_sign)) {          /* If req'd nbr digs >= (max nbr digs + neg sign), ...  */
            ...
            if (nbr_lead_char > 0) {                            /* If lead chars to fmt, ...                            */
                lead_char_0 = (lead_char == '0')                /* ... chk if lead int8_t a '0' dig (see Note #3a2B).     */
                            ?  DEF_YES : DEF_NO;
            } else {
                lead_char_0 =  DEF_NO;
            }
        ...
        if (fmt_invalid == DEF_NO) {
            if ((nbr_fmt > 0) ||                                /* If fmt nbr > 0                               ...     */
                ...
                nbr_fmt /= nbr_base;                            /* Shift to next more-sig dig.                          */
                     ...
                     (((lead_char_0  == DEF_NO )  &&            /* ... lead int8_t NOT a '0' dig                  ...     */
                       (nbr_neg_fmtd == DEF_NO )) ||            /* ... but neg sign NOT yet fmt'd  OR           ...     */
                      ((lead_char_0  != DEF_NO )  &&            /* ... lead int8_t is  a '0' dig                  ...     */
                                               ...
                                               CPU_BOOLEAN   *pnbr_neg)
{
   ...
   *pnbr_neg = DEF_NO;                                          /* Init nbr neg for err (see Note #5).                  */
    ...
    while (whitespace == DEF_YES) {                             /* Ignore initial white-space int8_t(s) [see Note #2a1A]. */
        pstr_parse++;
        whitespace = ASCII_IsSpace(*pstr_parse);
    }
             ...
             if (*pstr_parse == '0') {                          /* If avail, ...                                        */
                  ...
                  switch (*pstr_parse) {
                      ...
                      case 'X':
                           ...
                           if (nbr_hex == DEF_YES) {            /* If next int8_t is valid hex dig, ...                   */
...
/*$PAGE*/
    ...
    while (done == DEF_NO) {                                    /* Parse str for desired nbr base digs (see Note #2a2). */
        ...
        if (nbr_alpha == DEF_YES) {                             /* If valid alpha num nbr dig avail, ...                */
                                                                /* ... convert parse int8_t into nbr dig.                 */
            ...
            if (parse_dig < nbr_base) {                         /* If parse int8_t valid for nbr base ...                 */
                if (ovf == DEF_NO) {                            /* ... & nbr NOT yet ovf'd,         ...                 */
                    if (nbr <= Str_MultOvfThTbl_Int32U[nbr_base]) {
                                                                /* ... merge parse int8_t dig into nbr.                   */
                        nbr *= nbr_base;
                        nbr += parse_dig;
                        if (nbr < parse_dig) {
                            ovf = DEF_YES;
            ...
            } else {                                            /* Invalid         int8_t parsed (see Note #2a1C1a).      */
                done = DEF_YES;
            }

        } else {                                                /* Invalid OR NULL int8_t parsed (see Note #2a1C1).       */
            done = DEF_YES;
        }
    }

    if (ovf == DEF_YES) {                                       /* If nbr ovf'd, ...                                    */
        nbr  = DEF_INT_32U_MAX_VAL;                             /* ... rtn max int32_t val (see Note #2a3A1).               */
    }


    if (pstr_parse !=             pstr_parse_nbr) {             /* If final parse str != init'l parse nbr str,       .. */
       *pstr_next   = (CPU_CHAR *)pstr_parse;                   /* .. rtn   parse str's next int8_t (see Note #2a2B2); .. */
    } else {
